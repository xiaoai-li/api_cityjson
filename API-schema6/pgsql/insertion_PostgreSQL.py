...
def insert_cityjson(file_name, schema_name):
    ...
    for step_id, obj_index in enumerate(insert_order):
        ...
        if 'parents' in cityobject.keys():
            parent_id=cityobject['parents'][0]
        ...
        if ref_system:
            insert_cityobjects = """
            INSERT INTO cityobject (obj_id,bbox, type,attributes,parent_id, vertices, object, cityjson_id)
            VALUES (%s, '(({}, {}), ({}, {}))',%s, %s, %s,%s,%s,currval('cityjson_id_seq'))
            """.format(*min_xy_4236,*max_xy_4236)
            cur.execute(insert_cityobjects,
                        (obj_id, cityobject['type'], json.dumps(attributes),parent_id,
                         json.dumps(vertices),
