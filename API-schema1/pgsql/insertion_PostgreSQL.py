...
def insert_cityjson(file_name, schema_name):
    ...
    if ref_system:
        ...
        VALUES ( %s, %s, ST_MakeEnvelope({}, {}, {}, {}, {}), %s, %s,%s)""" \
        ...
        cur.execute(insert_metadata,(file_name,data_title, json.dumps(data['metadata']),json.dumps(data['transform'])))
    ...
    for step_id, obj_index in enumerate(insert_order):
        ...
        attributes['type']=cityobject['type']
        ...
        if ref_system:
            insert_cityobjects = """
            INSERT INTO cityobject (obj_id,bbox, parents, children, attributes, vertices, object, cityjson_id)
            VALUES (%s, ST_MakeEnvelope({}, {}, {}, {}, {}),%s, %s, %s, %s,%s, currval('cityjson_id_seq'))
            """.format(bbox2d[0], bbox2d[1], bbox2d[2], bbox2d[3], ref_system)
            cur.execute(insert_cityobjects,
                        (obj_id, parents, children, json.dumps(attributes),
                         json.dumps(vertices),
                         json.dumps(cityobject)))
        else:
            insert_cityobjects = """
            INSERT INTO cityobject (obj_id,parents, children, attributes, vertices, object, cityjson_id)
...
# # insert_cityjson('25dn2', DEFAULT_SCHEMA) # wrong dataset
...
# # insert_cityjson('25gz1', DEFAULT_SCHEMA) # wrong dataset
# # insert_cityjson('38an1', DEFAULT_SCHEMA)
# # insert_cityjson('38an1', DEFAULT_SCHEMA)#
#
# # s=timer()
# # insert_cityjson('25dz1', DEFAULT_SCHEMA) # need to be deleted
...
# #
# # insert_cityjson('31bn2', DEFAULT_SCHEMA) # wrong dataset
...
# # insert_cityjson('31bz2', DEFAULT_SCHEMA) # wrong dataset
...
#
# # insert_cityjson('30fn1', DEFAULT_SCHEMA) # need to be  insert
