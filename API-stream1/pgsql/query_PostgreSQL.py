    ...
    def generator():
        try:
            ...
            while True:
                ...
                """.format(schema_name, TOPLEVEL, query_bbox, CHUNK_SIZE, OFFSET)
                cur.execute(query_features, [cm_id])
                rows = cur.fetchall()
                if not rows:
                    break
                yield from __dumps_col(rows)
    ...
    def generator():
        try:
            ...
            while True:
                ...
                """.format(schema_name, query_attr, CHUNK_SIZE, OFFSET)
                cur.execute(query_features, [cm_id])
                rows = cur.fetchall()
                if not rows:
                    break
