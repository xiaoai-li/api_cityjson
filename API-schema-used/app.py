...
from pgsql.query_PostgreSQL import query_collections, query_items, query_feature, query_col, query_cols_bbox, \
    filter_col_bbox, query_col_transform

app = Flask(__name__)
DEFAULT_DB = 'cityjson'
DEFAULT_SCHEMA = 'addcolumns'
# jindex = json.loads(open('./datasets/index.json').read())
collections_info = query_collections(DEFAULT_SCHEMA)
...
def collection(dataset):
    ...
    if re_bbox is not None:
        ...
        try:
            ...
            return Response(
                stream_template('filtered_results.html', rows=generator, datasetname=dataset,
        ...
        generator = stream_with_context(filter_col_bbox(file_name=dataset, attrs=re_attrs))
        return Response(
            stream_template('filtered_results.html', rows=generator, datasetname=dataset, transform_int=transform_int,
        ...
        if re == 'html' or re is None:
            ...
            for each in collections:
                if each['name'] == dataset:
                    return render_template("collection.html", dataset=each, bounds=bbox_wgs84,
