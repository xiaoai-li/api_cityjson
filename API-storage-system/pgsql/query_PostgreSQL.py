...
def query_items(dataset_name=None, schema_name=DEFAULT_SCHEMA, limit=99999999, offset=0):
    ...
    for row in rows:
        add_cityobject(cm, row[0], row[1], row[2])
...
def query_item(dataset_name=None, schema_name=DEFAULT_SCHEMA, feature_id=None):
    ...
    query_cityfeature = """
            ...
            """.format(schema_name)
    cur.execute(query_cityfeature, [dataset_name, feature_id])
    ...
    for result in results:
        ...
        object = result[1]
        vertices = result[2]
        add_cityobject(cm, id, object, vertices)

    cm.remove_duplicate_vertices()
    cj_feature = {
        "type": "CityJSONFeature",
        "id": feature_id,
        "CityObjects": cm.j['CityObjects'],
